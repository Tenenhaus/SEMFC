...
#' display the betas/gammas between the latents nodes
...
#'
#' @examples
BuildCall_inner=function (model = model, 
                          display_numbers = TRUE,
                          graph_options = list(overlap = FALSE),
                          node_options_LV = list(shape = "oval", 
                                                 color = "black"), 
                          node_options_MV = list(shape = "box", 
                                                 color = "black"),
                          edge_options = list(penwidth = 0.5, color = "black"))
{
  ...
  ifelse(display_numbers, 
         string1 <- paste(string1, model_rgcca_inner(model, labels = TRUE)), 
         string1 <- paste(string1, model_rgcca_inner(model, labels = FALSE))
  )
