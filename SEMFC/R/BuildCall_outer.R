...
#'
#' @examples
BuildCall_outer=function (model = model, 
                          ...
                          edge_options = list(penwidth = 0.5, color = "black"),num=num)
{
  string1 <- ""
  ...
  string1 <- paste(string1, "graph [", paste(paste(names(graph_options), 
                   ...
                   "]")
  string1 <- paste(string1, "\n")
  nodes <- getNodes_outer(model,num)
  ...
  ifelse(display_numbers, 
         string1 <- paste(string1, model_rgcca_outer(model, labels = TRUE,num)), 
         string1 <- paste(string1, model_rgcca_outer(model, labels = FALSE,num))
  )
