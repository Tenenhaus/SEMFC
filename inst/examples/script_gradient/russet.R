...
cat("solnp vs nloptr:", round(result$pars - res0$solution, 4), "\n")
cat("solnp vs csolnp:", round(result$pars - sol$pars, 4), "\n")
cat("nloptr vs csolnp:", round(res0$solution - sol$pars, 4), "\n")


cat("Initial values vs nloptr:", round(init - res0$solution, 4), "\n")
...
# Afficher la différence maximale absolue
erreur_max_eq <- max(abs(jacob_analytique - jacob_numerique))
cat("Erreur max inf :", erreur_max_eq, "\n")