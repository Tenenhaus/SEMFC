...
li_GAMMA_TRUE <- lapply(1:R_true, function(r) {
    ...
    # vec_weights[1:(length(vec_weights) %/% 2)] <- 0.5
    annulations <- seq_len(n_endogenes * n_exogenes)[seq_len(n_endogenes * n_exogenes) %% (4) %in% c(1, 3)]
    vec_weights[annulations] <- 0
    # print(vec_weights)
...
li_R22_TRUE <- lapply(1:R_true, function(r) {
    vec_values <- rep(1 / sqrt(2), n_endogenes^2) / r
...
li_PHI_TRUE <- lapply(1:R_true, function(r) {
    vec_values <- rep(0.5 / r, n_exogenes^2)
