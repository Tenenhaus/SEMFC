...
Y <- list(clinique = X[, 1:5], biologique = X[, 6:11])
```
...
eta2 =~ polig+micro+mog+ftl+pmog+pftl
...
```
```{r}
summary(fit.sem.ml, fit.measures = T)
```
...
gof = svdSEM_gof(model$svd_parameters, B = 2000) #~half of improper solutions
...
std_all_lavaan = estimate[1:11,11]

lambda_comparaison = cbind(estimate[1:11,1:3],std_all_svd, std_all_ml, std_all_lavaan)
print('lambda')
print(lambda_comparaison)
```
```{r}
cor(std_all_ml, std_all_svd)
```
```{r}
...
bg_lavaan = estimate[12,11]
beta_gama_comparaison = cbind(estimate[12,1:3], bg_svd, bg_ml, bg_lavaan)
print('beta et gamma')
...
res_var_lavaan = estimate[13:23,11]


res_var_comparaison = cbind(estimate[13:23,1:3], res_var_svd, res_var_ml, res_var_lavaan)
